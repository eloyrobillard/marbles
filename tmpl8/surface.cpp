...
void Surface::Plot(float x, float y, Pixel c) {
	...
	float dx = fmodf(x, xi), dy = fmodf(y, yi);

	int r = (c & 0xff0000) >> 16, g = (c & 0xff00) >> 8, b = c & 0xff;

	if ((xi >= 0) && (yi  >= 0) && (xi  < m_Width) && (yi  < m_Height)) {
		float coeff = (1 - dx) * (1 - dy);
		m_Buffer[xi + yi * m_Pitch] = ((int)(r * coeff) << 16) + ((int)(g * coeff) << 8) + (int)(b*coeff);
	}

	if ((xi >= 0) && (yi + 1  >= 0) && (xi < m_Width) && (yi + 1 < m_Height)) {
		float coeff = (1 - dx) * dy;
		m_Buffer[xi + (yi+1) * m_Pitch] = ((int)(r * coeff) << 16) + ((int)(g * coeff) << 8) + (int)(b*coeff);
	}

	if ((xi + 1 >= 0) && (yi >= 0) && (xi + 1 < m_Width) && (yi < m_Height)) {
		float coeff = (1 - dy) * dx;
		m_Buffer[xi + 1 + yi * m_Pitch] = ((int)(r * coeff) << 16) + ((int)(g * coeff) << 8) + (int)(b*coeff);
	}

	if ((xi + 1 >= 0) && (yi + 1 >= 0) && (xi + 1 < m_Width) && (yi + 1 < m_Height)) {
		float coeff = dy * dx;
		m_Buffer[xi + 1 + (yi+1) * m_Pitch] = ((int)(r * coeff) << 16) + ((int)(g * coeff) << 8) + (int)(b*coeff);
	}
}
