...
float getShadow(vec4 lightSpace, sampler2D shadowMap) {
  ...
  for (int i = 0; i < 9; i++) {
    ...
    // HACK: 本当にこれでいいの？これだと光に近いサンプルはとりあえず影として解釈する
    // より遠いほうはそうでないと
...
void main() {
  ...
  if (distance(fragWorldPos, cameraPos) <= 12.f) {
    float marbleShadow = getShadow(fragPosLightSpaceMarble, shadowMapNear);
    if (marbleShadow > 0.0) {
      outColor = color + vec4(vec3(0.002), marbleShadow);
      return;
    }
  } else {
    float staticShadow = getShadow(fragPosLightSpaceStatic, shadowMapFar);
